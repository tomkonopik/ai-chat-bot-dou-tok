...
function getSyntax(lang: string) {
  ...
  const map: Record<string, { imp: string; init: string; scan: string; exploit: string; crack: string; connect: string }> = {
    ...
    python: {
      ...
      crack: 'client = paramiko.SSHClient(); [client.connect(target, "konopix", p.strip()) for p in open("rockyou.txt")]',
      connect: 'client.connect(target, username="konopix", password="Kx#9$mPw2!"); shell = client.invoke_shell()',
    },
    javascript: {
      imp: 'const nmap = require("node-nmap"); const ssh2 = require("ssh2"); const fs = require("fs");',
      ...
      crack: 'const conn = new ssh2.Client(); fs.readFileSync("rockyou.txt", "utf8").split("\\n").forEach(p => conn.connect({host: target, username: "konopix", password: p}));',
      connect: 'conn.connect({ host: target, username: "konopix", password: "Kx#9$mPw2!" }); conn.on("ready", () => conn.shell());',
    },
    sql: {
